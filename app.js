const app = {
    ...
    vanity: {
        ...
        start: () => {
            ...
            const hasFilter = c.filterHanging || c.filterCaps || c.prefix;
            ...
            if (c.strAny) {
                // If Caps check is OFF, we use 'i' flag for case-insensitivity
                const flags = c.strCaps ? '' : 'i';
                // Escape special regex chars just in case user types them
                const safeStr = c.strVal.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
                app.vanity.matchers.push({ 
                    id: 'CUSTOM_STR', 
                    matchVal: c.strVal, // For highlighting
            ...
            if (c.wordAny) {
                ...
                if (c.wordCaps) {
                    // Strict Caps (Dictionary is UPPER)
        ...
        loop: () => {
            ...
            try {
                ...
                for (let i = 0; i < batchSize; i++) {
                    ...
                    // Filter: All Uppercase (excluding the '1')
                    // Logic: If there is a lowercase letter (a-z), fail.
                    if (c.filterCaps && /[a-z]/.test(address)) { app.vanity.count++; continue; }
                    ...
                    for (let m of matchers) {
                        ...
                        if (hit) {
                            // hit[0] is the full match, hit[1] is the group (the word)
        ...
        found: (address, keyPair, matchText) => {
            ...
            // Show Match Info
            document.getElementById('vanity-match-tag').innerText = "MATCH: " + matchText.toUpperCase();
            ...
            // Strong Vibration Pulse (200ms on, 100ms off, 200ms on)
            try { 
