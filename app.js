const app = {
    ...
    vanity: {
        ...
        // Dictionary (Condensed for performance)
        ...
        config: {},
            ...
            const c = {
                ...
                filterCaps: document.getElementById('v-caps').checked,
                
                prefix: document.getElementById('v-prefix').value.trim(),
            ...
            // 2. VALIDATION
            if (!c.prefix && !c.wordStart && !c.strAny && !c.wordAny) return alert("Select at least one Search Rule.");
            if (c.strAny && !c.strVal) return alert("Enter a string for 'Find String Anywhere'.");
            if (c.prefix && /[^1-9A-HJ-NP-Za-km-z]/.test(c.prefix)) return alert("Invalid Prefix characters.");

            // 3. PRE-COMPILE REGEX
            app.vanity.matchers = [];

            // A. Starts With Word (FIXED: Now supports mixed case "Bank")
            if (c.wordStart) {
                // We use the 'i' flag here so 1Bank matches BANK
                const words = app.vanity.dictionary.split(',').filter(w => w.length >= 3);
                // Note: We use 'i' flag to find "Bank" or "BANK"
                app.vanity.matchers.push({ type: 'regex', rx: new RegExp(`^1(${words.join('|')})`, 'i') });
            }

            // B. Word Anywhere (Already Correct)
            if (c.wordAny) {
                let words = app.vanity.dictionary.split(',').filter(w => w.length >= c.wordMin);
                if (c.wordCaps) {
                    // Force Exact Case (Dictionary is UPPER, so this only finds UPPER)
                    app.vanity.matchers.push({ type: 'regex', rx: new RegExp(`(${words.join('|')})`) });
                } else {
                    // Case Insensitive (Finds "Bank" using "BANK")
                    app.vanity.matchers.push({ type: 'regex', rx: new RegExp(`(${words.join('|')})`, 'i') });
                }
            }
        ...
        loop: () => {
            ...
            const batchSize = 200; // Addresses per frame
            ...
            try {
                ...
                for (let i = 0; i < batchSize; i++) {
                    ...
                    // 2. CHECK GLOBAL FILTERS (Fastest Fail First)
                    
                    // Filter: No Hanging (g, j, p, q, y)
                    if (c.filterHanging) {
                        if (/[gjpqy]/.test(address)) { app.vanity.count++; continue; }
                    }

                    // Filter: All Uppercase (No a-z)
                    if (c.filterCaps) {
                        if (/[a-z]/.test(address)) { app.vanity.count++; continue; }
                    }

                    // 3. CHECK SEARCH RULES (Any Match = Success)
                    let found = false;

                    // Rule A: Prefix
                    if (c.prefix && address.startsWith("1" + c.prefix)) found = true;

                    // Rule B: Custom String Anywhere
                    if (!found && c.strAny) {
                        // If "Caps Only" for string is on, we check case-sensitive
                        // If off, we check case-insensitive
                        if (c.strCaps) {
                            if (address.includes(c.strVal)) found = true;
                        } else {
                            if (address.toLowerCase().includes(c.strVal.toLowerCase())) found = true;
                        }
                    }

                    // Rule C & D: Dictionary Matchers (Regex)
                    if (!found && matchers.length > 0) {
                        for (let m of matchers) {
                            if (m.rx.test(address)) { found = true; break; }
                        }
                    }
